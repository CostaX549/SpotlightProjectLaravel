...
$(document).ready(function() {
    ...
    function fetchPopularMovies(page) {
      const popularMoviesUrl = `https://api.themoviedb.org/3/movie/popular?api_key=${apiKey}&language=pt-BR&page=${page}&certification_country=BR&certification.lte=12`;
        ...
        .then(data => {
          ...
          data.results.forEach((movie, index) => {
            ...
            const img = $('<img>').attr('src', `https://image.tmdb.org/t/p/w500/${posterPath}`)
                                   .attr('alt', title)
                                   .addClass('d-block w-100 rounded mb-4') // Adicione a classe 'rounded' para o border-radius
            col.append(img);
            ...
            img.click(function () {
            // Redirecionar para a página de detalhes do filme no Laravel
            window.location.href = `/filmes/${movie.id}`;
            });
    ...
    let currentSeriesPage = 5; // Começar a busca a partir da página 2
      ...
      fetch(seriesUrl)
        ...
        .then(data => {
          ...
          data.results.forEach((serie, index) => {
            ...
            if (colCounter === 0) {
              carouselItem = $('<div>').addClass('carousel-item');
              if (index === 0 && page === 5) { // Verifique se a página é 2 para adicionar a classe "active"
                carouselItem.addClass('active');
              }
    ...
    function fetchDocumentaries(page) {
      ...
      fetch(documentariesUrl)
        .then(response => response.json())
        .then(data => {
          const prevControl = $('.carousel-control-prev');
